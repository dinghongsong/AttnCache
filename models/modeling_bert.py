...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if collect_hiddenstates_apms:
            ...
            with open(f"{save_dir}/APMsDB/{files_num}.pickle", "wb") as file:
                pickle.dump(attention_probs.cpu().detach().numpy(), file)
    ...
    def forward_attention_cache(
        ...
        if compute_tensor_index.size:  
            ...
            ############################################
            mixed_query_layer = self.query(part_hidden_states)
            ...
            if is_cross_attention and past_key_value is not None:
                ...
                key_layer = self.transpose_for_scores(self.key(part_hidden_states))
                value_layer = self.transpose_for_scores(self.value(part_hidden_states))
                key_layer = torch.cat([past_key_value[0], key_layer], dim=2)
                value_layer = torch.cat([past_key_value[1], value_layer], dim=2)
            else:
                key_layer = self.transpose_for_scores(self.key(part_hidden_states))
            ...
            if attention_mask is not None:
                # Apply the attention mask is (precomputed for all layers in BertModel forward() function)
                attention_scores = attention_scores + attention_mask[compute_tensor_index]

            # Normalize the attention scores to probabilities.
            attention_probs = nn.functional.softmax(attention_scores, dim=-1)

            for idx, i in zip(compute_tensor_index, range(len(compute_tensor_index))):
                attention_cache[idx] = attention_probs[i]

        attention_probs = self.dropout(attention_cache)
        ################################################################
...
class BertAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        # )
        self.self = BERT_SELF_ATTENTION_CLASSES["eager"](
            config, position_embedding_type=position_embedding_type
        )
