...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        # self.attention_cache = torch.empty((batch_size, self.num_heads, seq_length, seq_length), dtype=torch.float32) # used for atten
    ...
    def forward(
        ...
        position_embeddings: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,  # will become mandatory in v4.46
        ########################
        collect_hiddenstates_apms: Optional[bool] = False,
        ...
        **kwargs,
        ...
        if attention_cache is not None:
            return self.forward_attention_cache(hidden_states=hidden_states,
        ...
        **kwargs,
        ...
        if compute_tensor_index.size:  
            ...
            for idx, i in zip(compute_tensor_index, range(len(compute_tensor_index))):
                attention_cache[idx] = attention_probs_compute[i]
    
        
        attn_output = torch.matmul(attention_cache[:value_states.shape[0], :, :, :], value_states)
        attn_output = attn_output.transpose(1, 2).contiguous()
        attn_output = attn_output.reshape(bsz, q_len, -1)
        attn_output = self.o_proj(attn_output)
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        #################################
        self.layer_idx = layer_idx
        self.is_attn_drop = config.is_attn_drop
        ########################
    def forward(
        self,
        hidden_states: torch.Tensor,
        ...
        **kwargs,
        ...
        # if self.is_attn_drop:
        #     return self.attn_drop_forward(
        #             hidden_states,
        #             attention_mask,
        #             position_ids,
        #             past_key_value,
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            ...
            # compute_tensor_index = compute_tensor_index,
            ########################
            collect_hiddenstates_apms=collect_hiddenstates_apms,
            save_dir = save_dir,
            attention_cache=attention_cache,
            compute_tensor_index=compute_tensor_index,
        ...
        **kwargs,
        ...
        Args:
            ...
            kwargs (`dict`, *optional*):
                ...
                into the model
        """
        if self.layer_idx not in [20, 21, 22, 23]:
            residual = hidden_states
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        #################################
        self.is_block_drop = config.is_block_drop
        self.config = config
        ########################
    ...
    def forward(
        ...
        collect_hiddenstates_apms: Optional[bool] = False,
        save_dir: Optional[str] = "",
        attention_cache: Optional[torch.FloatTensor] = None,
        compute_tensor_index: Optional[torch.FloatTensor] = None,
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs, self_attention_time = decoder_layer(
                    ...
                    collect_hiddenstates_apms=collect_hiddenstates_apms,
                    save_dir = save_dir,
                    attention_cache=attention_cache[i] if attention_cache is not None else None,
                    compute_tensor_index=compute_tensor_index,
    ...
    def block_drop_forward(
        ...
        for decoder_layer in self.layers:
            ####################
            if layer_cnt in block_drop_layer:
                continue
            layer_cnt += 1
